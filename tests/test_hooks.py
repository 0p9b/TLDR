...
class HookScriptTests(unittest.TestCase):
    ...
    def test_install_upgrades_old_two_file_install(self):
        with tempfile.TemporaryDirectory(prefix="tldr-hooks-upgrade-") as tmp:
            ...
            (hooks_dir / "tldr-activate.js").write_text("")
            (hooks_dir / "tldr-mode-tracker.js").write_text("")

            self.run_cmd(["bash", "src/hooks/install.sh"], home)

            statusline = hooks_dir / "tldr-statusline.sh"
    ...
    def test_install_reconfigures_missing_statusline(self):
        with tempfile.TemporaryDirectory(prefix="tldr-hooks-statusline-") as tmp:
            ...
            for name in ("tldr-activate.js", "tldr-mode-tracker.js", "tldr-statusline.sh"):
                    ...
                    "SessionStart": [
                        {
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": f'node "{hooks_dir / "tldr-activate.js"}"',
                                }
                            ]
                        }
                    ],
                    "UserPromptSubmit": [
                        {
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": f'node "{hooks_dir / "tldr-mode-tracker.js"}"',
                                }
                            ]
                        }
                    ],
