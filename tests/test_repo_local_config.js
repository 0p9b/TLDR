...
//   env CAVEMAN_DEFAULT_MODE → repo-local (.tldr/config.json or .tldr.json,
...
test('reads .tldr/config.json in cwd', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.tldr'));
...
test('reads .tldr.json in cwd', (tmp) => {
  fs.writeFileSync(path.join(tmp, '.tldr.json'),
    JSON.stringify({ defaultMode: 'ultra' }));
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'ultra');
});

test('.tldr/config.json wins over .tldr.json at same level', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.tldr'));
  fs.writeFileSync(path.join(tmp, '.tldr', 'config.json'),
    JSON.stringify({ defaultMode: 'lite' }));
  fs.writeFileSync(path.join(tmp, '.tldr.json'),
...
test('repo-local config beats user config', (tmp) => {
  ...
  fs.mkdirSync(path.join(tmpHome, 'tldr'), { recursive: true });
  fs.writeFileSync(path.join(tmpHome, 'tldr', 'config.json'),
  ...
  try {
    ...
    fs.rmSync(path.join(tmpHome, 'tldr'), { recursive: true, force: true });
  }
});
...
test('malformed JSON in repo config falls through to default', (tmp) => {
  fs.mkdirSync(path.join(tmp, '.tldr'));
  fs.writeFileSync(path.join(tmp, '.tldr', 'config.json'), '{ not json');
  process.chdir(tmp);
  assert.strictEqual(getDefaultMode(), 'full');
});

test('refuses symlinked .tldr.json (symmetric with readFlag policy)', (tmp) => {
  const real = path.join(tmp, 'real-config.json');
  fs.writeFileSync(real, JSON.stringify({ defaultMode: 'ultra' }));
  try {
    fs.symlinkSync(real, path.join(tmp, '.tldr.json'));
  } catch (e) {
