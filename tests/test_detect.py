"""Tests for detect.py file-type classification.

Extensionless build files (`Dockerfile`, `Makefile`), `.txt`-suffixed build
and dependency files (`CMakeLists.txt`, `requirements.txt`), and shebang
scripts used to fall through to the content heuristic and come back as
compressible natural language — so `/tldr-compress Dockerfile` would overwrite
a Dockerfile with compressed prose. These tests pin the basename, prefix,
dependency-manifest, and shebang guards.
"""
...
class DetectFileTypeTests(unittest.TestCase):
    ...
    def test_dockerfile_variant_is_code(self):
        with tempfile.TemporaryDirectory() as tmp:
            for name in ("Dockerfile.prod", "Containerfile.dev", "Containerfile"):
                p = self._write(tmp, name, DOCKERFILE_BODY)
                self.assertEqual(detect_file_type(p), "code", name)
                self.assertFalse(should_compress(p), name)
    ...
    def test_requirements_txt_is_config_not_compressible(self):
        with tempfile.TemporaryDirectory() as tmp:
            for name in ("requirements.txt", "requirements-dev.txt", "constraints.txt"):
                p = self._write(tmp, name, "requests==2.31.0\nurllib3>=2\n")
                self.assertEqual(detect_file_type(p), "config", name)
                self.assertFalse(should_compress(p), name)
