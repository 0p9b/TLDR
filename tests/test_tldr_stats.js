...
test('--share prints single-line tweetable summary', (tmp) => {
  ...
  assert.match(out, /^🦉 Saved 650 output tokens \(~\$0\.009[78]\) across 1 turns this session — github\.com\/0p9b\/TLDR$/m);
});
...
test('--share works with no benchmark ratio (lite mode)', (tmp) => {
  ...
  assert.match(out, /^🦉 1 turns, 200 output tokens this session — github\.com\/0p9b\/TLDR$/m);
});
