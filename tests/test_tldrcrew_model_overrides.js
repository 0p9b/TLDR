#!/usr/bin/env node
// Tests for src/hooks/tldrcrew-model-overrides.js
// Run: node tests/test_tldrcrew_model_overrides.js
...
test('preserves all other frontmatter lines', () => {
  ...
  assert.ok(out.includes('name: tldrcrew-reviewer'), 'name line lost');
...
function withTmpPlugin(fn) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'tldr-override-test-'));
...
test('replaces reviewer model when TLDRCREW_REVIEWER_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), 'utf8');
    assert.ok(out.includes('model: sonnet'), 'reviewer model not patched');
  });
});

test('replaces investigator model when TLDRCREW_INVESTIGATOR_MODEL set', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-investigator.md'), INVESTIGATOR_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_INVESTIGATOR_MODEL: 'opus' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-investigator.md'), 'utf8');
    assert.ok(out.includes('model: opus'), 'investigator model not patched');
  });
});

test('inserts builder model when TLDRCREW_BUILDER_MODEL set and no model line', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-builder.md'), BUILDER_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_BUILDER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-builder.md'), 'utf8');
...
test('blank env var is no-op', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_REVIEWER_MODEL: '' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), 'utf8');
...
test('env var with newline in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_REVIEWER_MODEL: 'so\nnnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), 'utf8');
...
test('env var with control character in value is ignored', () => {
  withTmpPlugin((root, agentsDir) => {
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), REVIEWER_FM, 'utf8');
    applyOverrides(root, { TLDRCREW_REVIEWER_MODEL: 'son\x00net' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), 'utf8');
...
test('missing agents dir (non-plugin layout) is silent no-op', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'tldr-nolayout-'));
  try {
    assert.doesNotThrow(() => {
      applyOverrides(tmp, { TLDRCREW_REVIEWER_MODEL: 'sonnet' });
    });
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
...
test('body content preserved after model patch', () => {
  withTmpPlugin((root, agentsDir) => {
    ...
    fs.writeFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), content, 'utf8');
    applyOverrides(root, { TLDRCREW_REVIEWER_MODEL: 'sonnet' });
    const out = fs.readFileSync(path.join(agentsDir, 'tldrcrew-reviewer.md'), 'utf8');
...
test('AGENT_ENV_MAP covers all three tldrcrew agents', () => {
  const files = AGENT_ENV_MAP.map(e => path.basename(e.file)).sort();
