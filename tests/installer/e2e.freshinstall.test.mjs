...
test('fresh install populates hooks dir and settings.json (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; the claude provider is the only path that wires hooks' }, () => {
  ...
  try {
    ...
    assert.ok(fs.existsSync(path.join(hooks, 'tldr-activate.js')),     'tldr-activate.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'tldr-mode-tracker.js')), 'tldr-mode-tracker.js missing');
    assert.ok(fs.existsSync(path.join(hooks, 'tldr-config.js')),       'tldr-config.js missing');
    ...
    assert.match(getStatuslineCommand(settings), /tldr-statusline/,
      'statusLine command does not reference TLDR');
  } finally {
...
test('uninstall strips TLDR hooks but preserves user-authored ones (skipped without `claude` CLI)', { skip: !hasClaudeCli() && 'claude CLI not on PATH; uninstall test depends on a prior real install' }, () => {
  const dir = freshTmpDir();
  try {
    ...
    if (fs.existsSync(hooks)) {
      for (const f of ['tldr-activate.js', 'tldr-mode-tracker.js', 'tldr-config.js', STATUSLINE_FILE]) {
    ...
    // Settings cleaned up.
    const settings = JSON.parse(fs.readFileSync(path.join(dir, 'settings.json'), 'utf8'));
    // No remaining tldr-marked hooks anywhere.
    for (const ev of Object.keys(settings.hooks || {})) {
      const arr = settings.hooks[ev] || [];
      for (const e of arr) {
        for (const h of (e.hooks || [])) {
          assert.doesNotMatch(h.command || '', /tldr/, `${ev} still has TLDR hook: ${h.command}`);
        }
      }
    }
...
const SKILL_BODY_SRC = path.join(REPO_ROOT, 'skills', 'tldr', 'SKILL.md');
...
test('openclaw install writes skill folder + SOUL.md bootstrap', () => {
  ...
  try {
    ...
    const skillFile = path.join(ws, 'skills', 'tldr', 'SKILL.md');
...
test('openclaw install is idempotent: skill frontmatter not double-prepended, SOUL.md has one marker block', () => {
  ...
  try {
    ...
    const beginMatches = soulRaw.match(/<!-- tldr-begin -->/g) || [];
...
test('tldr-init.js --only openclaw routes through the same helper', () => {
  const dir = freshTmpDir();
  const ws = path.join(dir, 'ws');
  fs.mkdirSync(ws);
  try {
...
test('lib settings.addCommandHook is idempotent across two synthetic install passes', () => {
  ...
  try {
    ...
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/abs/hooks/tldr-activate.js"',
      marker: 'tldr-activate',
    });
    SETTINGS.addCommandHook(settings, 'SessionStart', {
      command: '"/usr/bin/node" "/different/hooks/tldr-activate.js"',
      marker: 'tldr-activate',
    });
