#!/bin/bash
# TLDR — uninstaller for the SessionStart + UserPromptSubmit hooks
# Removes: hook files in ~/.claude/hooks, settings.json entries, and the flag file
# Usage: bash src/hooks/uninstall.sh
#   or:  bash <(curl -s https://raw.githubusercontent.com/ZeroPointNineBar/TLDR/main/src/hooks/uninstall.sh)
set -e
...
if [ -d "$CLAUDE_DIR/plugins" ]; then
  if find "$CLAUDE_DIR/plugins" -path "*/tldr*" -name "plugin.json" -print -quit 2>/dev/null | grep -q .; then
    PLUGIN_INSTALLED=1
  fi
fi

if [ "$PLUGIN_INSTALLED" -eq 1 ]; then
  echo "TLDR appears to be installed as a Claude Code plugin."
  echo "To uninstall the plugin, run:"
  echo ""
  echo "  claude plugin disable TLDR"
  echo ""
...
if [ -f "$SETTINGS" ]; then
  ...
  if ! command -v node >/dev/null 2>&1; then
    ...
    echo "         Remove the TLDR SessionStart and UserPromptSubmit"
    ...
    TLDR_SETTINGS="$SETTINGS" TLDR_HOOKS_DIR="$HOOKS_DIR" node -e "
      const fs = require('fs');
      ...
      // Remove statusLine if it references TLDR
...
echo ""
echo "Other agents:"
echo "  npx skills remove TLDR    # Cursor, Windsurf, Cline, Copilot, etc."
echo "  claude plugin disable TLDR  # Claude Code plugin"
echo "  gemini extensions uninstall TLDR  # Gemini CLI"
