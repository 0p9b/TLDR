# TLDR — uninstaller for the SessionStart + UserPromptSubmit hooks (Windows PowerShell)
...
#   or:  irm https://raw.githubusercontent.com/ZeroPointNineBar/TLDR/main/src/hooks/uninstall.ps1 | iex
param()
...
if ($PluginInstalled) {
    Write-Host "TLDR appears to be installed as a Claude Code plugin." -ForegroundColor Yellow
    Write-Host "To uninstall the plugin, run:"
    Write-Host ""
    Write-Host "  claude plugin disable TLDR" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host "Uninstalling TLDR hooks..."
...
# 2. Remove TLDR entries from settings.json (idempotent)
if (Test-Path $Settings) {
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
        Write-Host "WARNING: 'node' not found - cannot safely edit settings.json." -ForegroundColor Yellow
        Write-Host "         Remove the TLDR SessionStart and UserPromptSubmit"
...
if (settings.statusLine) {
  ...
  if (cmd.includes(managedStatusLinePath)) {
    ...
    console.log('  Removed TLDR statusLine from settings.json');
  }
}

fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 2) + '\n');
console.log('  Removed ' + removed + ' TLDR hook entries from settings.json');
'@
...
Write-Host "  npx skills remove TLDR      # Cursor, Windsurf, Cline, Copilot, etc."
Write-Host "  claude plugin disable TLDR   # Claude Code plugin"
Write-Host "  gemini extensions uninstall TLDR  # Gemini CLI"
