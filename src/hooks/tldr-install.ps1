# TLDR — one-command hook installer for Claude Code (Windows PowerShell)
...
#        irm https://raw.githubusercontent.com/0p9b/TLDR/main/src/hooks/install.ps1 | iex
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: 'node' is required to install the TLDR hooks (used to merge" -ForegroundColor Red
...
$RepoUrl = "https://raw.githubusercontent.com/0p9b/TLDR/main/src/hooks"
...
if (-not $Force) {
    ...
    if ($AllFilesPresent -and $HooksWired -and $HasStatusLine) {
        Write-Host "TLDR hooks already installed in $HooksDir"
...
if ($Force -and (Test-Path (Join-Path $HooksDir "tldr-activate.js"))) {
    Write-Host "Reinstalling TLDR hooks (-Force)..."
} else {
    Write-Host "Installing TLDR hooks..."
}
...
$env:TLDR_SETTINGS = $Settings -replace '\\', '/'
$env:TLDR_HOOKS_DIR = $HooksDir -replace '\\', '/'

$nodeScript = @'
const fs = require('fs');
const settingsPath = process.env.TLDR_SETTINGS;
const hooksDir = process.env.TLDR_HOOKS_DIR;
const managedStatusLinePath = hooksDir + '/tldr-statusline.ps1';
...
if (!hasStart) {
  settings.hooks.SessionStart.push({
    hooks: [{
      type: 'command',
      command: 'node "' + hooksDir + '/tldr-activate.js"',
      timeout: 5,
...
if (!hasPrompt) {
  settings.hooks.UserPromptSubmit.push({
    hooks: [{
      type: 'command',
      command: 'node "' + hooksDir + '/tldr-mode-tracker.js"',
      timeout: 5,
...
if (!settings.statusLine) {
  ...
  if (cmd.includes(managedStatusLinePath)) {
    ...
    console.log('  NOTE: Existing statusline detected - TLDR badge NOT added.');
...
Write-Host "  - SessionStart hook: auto-loads TLDR rules every session"
Write-Host "  - Mode tracker hook: updates statusline badge when you switch modes"
Write-Host "    (/tldr lite, /tldr ultra, /tldr wenyan, /tldr-commit, etc.)"
Write-Host "  - Statusline badge: shows [TLDR] or [TLDR:ULTRA] etc."
