...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '🦉 TLDR armed but no turns yet — github.com/0p9b/TLDR';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `🦉 Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — github.com/0p9b/TLDR`;
  }
  return `🦉 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — github.com/0p9b/TLDR`;
}
