#!/usr/bin/env node
// tldr-stats — read the active Claude Code session log, print real token
// usage plus an estimated savings figure from the benchmark in benchmarks/.
//
// Run directly:    node hooks/tldr-stats.js
// Inside Claude:   /tldr-stats triggers this via the UserPromptSubmit hook.
...
const os = require('os');
const { readFlag, appendFlag, readHistory, safeWriteFlag } = require('./tldr-config');
...
// Detect *.original.md / *.md pairs left behind by tldr-compress. The
...
function formatHistory({ sessions, outputTokens, estSavedTokens, estSavedUsd, since }) {
  ...
  if (sessions === 0) {
    return `\nTLDR Stats — Lifetime${window}\n${sep}\nNo sessions logged yet — run /tldr-stats inside any session to start tracking.\n${sep}\n`;
  }
...
function formatShare({ outputTokens, turns, mode, model }) {
  if (turns === 0) {
    return '🦉 TLDR armed but no turns yet — github.com/0point9bar/TLDR';
  }
  const ratio = COMPRESSION[mode] != null ? COMPRESSION[mode] : null;
  const price = priceForModel(model);
  ...
  if (ratio !== null) {
    ...
    return `🦉 Saved ${estSaved.toLocaleString()} output tokens${usd} across ${turns} turns this session — github.com/0point9bar/TLDR`;
  }
  return `🦉 ${turns} turns, ${outputTokens.toLocaleString()} output tokens this session — github.com/0point9bar/TLDR`;
}
...
function formatStats({ outputTokens, cacheReadTokens, turns, mode, model, sessionPath, compressed }) {
  ...
  if (ratio !== null) {
    ...
    savings = 'TLDR not active this session.';
  }
...
function main() {
  ...
  if (all || sinceArg) {
    ...
    if (sinceArg && sinceMs === null) {
      process.stderr.write(`tldr-stats: --since takes Nh or Nd (e.g. 7d, 24h), got: ${sinceArg}\n`);
      process.exit(2);
    }
  ...
  const mode = readFlag(path.join(claudeDir, '.tldr-active'));

  // Append a snapshot of this session's totals to the lifetime log. Multiple
  // /tldr-stats calls in one session emit multiple lines for the same
  ...
  if (parsed.turns > 0) {
    ...
    // cat without parsing JSONL. Updated on every /tldr-stats run.
    // Routed through safeWriteFlag — the suffix path is predictable and
    // user-owned, same symlink-clobber surface as the .tldr-active flag.
