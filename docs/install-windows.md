...
If `irm https://raw.githubusercontent.com/ZeroPointNineBar/TLDR/main/install.ps1 | iex` fails on Windows (issues #249, #199, #72), set up plugin-skill activation by hand. This does **not** install the standalone hooks or the statusline — for those, run the unified Node installer afterwards: `npx -y github:ZeroPointNineBar/TLDR -- --only claude` (or `node bin/install.js --only claude` from a clone).
...
$plugins["tldr"] = [ordered]@{ name = "tldr"; source = "ZeroPointNineBar/TLDR"; version = "main" }
...
Verify: `Test-Path "$PluginSkillDir\SKILL.md"` should print `True`. Restart Claude Code, then run `/tldr` to confirm the skill loads.
...
```powershell
npx skills add ZeroPointNineBar/TLDR --copy
```
...
Code/commits/PRs: normal. Off: "stop tldr" / "normal mode".
```
