...
```bash
npx -y github:ZeroPointNineBar/TLDR -- --dry-run
```
...
|---|---|:-:|
| **Claude Code** | `claude plugin marketplace add ZeroPointNineBar/TLDR && claude plugin install tldr@tldr` | Yes |
| **Gemini CLI** | `gemini extensions install https://github.com/ZeroPointNineBar/TLDR` | Yes |
| **opencode** | `node bin/install.js --only opencode` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only opencode`)* | Yes (plugin + AGENTS.md) |
| **OpenClaw** | `npx -y github:ZeroPointNineBar/TLDR -- --only openclaw` | Yes (workspace skill + SOUL.md) |
| **Codex CLI** | `node bin/install.js --only codex` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only codex`)* | Yes (global `~/.codex/AGENTS.md` + skill) |
| **Pi Coding Agent** | `node bin/install.js --only pi` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only pi`)* | Yes (global `~/.pi/agent/AGENTS.md` + skill) |
| **Grok Build CLI** | `node bin/install.js --only grok` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only grok`)* | Yes (global `~/.grok/AGENTS.md` + skill) |
| **Cursor** | `node bin/install.js --only cursor` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only cursor`)* | Skill (`/tldr`) at `~/.cursor/skills/`; cursor-agent has no global rules file, so always-on is per-repo via `--with-init` |
| **Windsurf** | `npx skills add ZeroPointNineBar/TLDR -a windsurf` | Per-session by default; `--with-init` for an always-on rule file |
| **Cline** | `npx skills add ZeroPointNineBar/TLDR -a cline` | Per-session by default; `--with-init` for an always-on rule file |
| **GitHub Copilot** *(soft probe)* | `npx -y github:ZeroPointNineBar/TLDR -- --only copilot --with-init` | Repo-wide instructions via `--with-init` |
| **Continue** | `npx skills add ZeroPointNineBar/TLDR -a continue` | No — say `/tldr` |
| **Kilo Code** | `npx skills add ZeroPointNineBar/TLDR -a kilo` | No |
| **Roo Code** | `npx skills add ZeroPointNineBar/TLDR -a roo` | No |
| **Augment Code** | `npx skills add ZeroPointNineBar/TLDR -a augment` | No |
| **Aider Desk** | `npx skills add ZeroPointNineBar/TLDR -a aider-desk` | No |
| **Sourcegraph Amp** | `npx skills add ZeroPointNineBar/TLDR -a amp` | No |
| **IBM Bob** | `npx skills add ZeroPointNineBar/TLDR -a bob` | No |
| **Crush** | `npx skills add ZeroPointNineBar/TLDR -a crush` | No |
| **Devin (terminal)** | `npx skills add ZeroPointNineBar/TLDR -a devin` | No |
| **Droid (Factory)** | `npx skills add ZeroPointNineBar/TLDR -a droid` | No |
| **ForgeCode** | `npx skills add ZeroPointNineBar/TLDR -a forgecode` | No |
| **Block Goose** | `npx skills add ZeroPointNineBar/TLDR -a goose` | No |
| **iFlow CLI** | `npx skills add ZeroPointNineBar/TLDR -a iflow-cli` | No |
| **Kiro CLI** | `npx skills add ZeroPointNineBar/TLDR -a kiro-cli` | No |
| **Mistral Vibe** | `npx skills add ZeroPointNineBar/TLDR -a mistral-vibe` | No |
| **OpenHands** | `npx skills add ZeroPointNineBar/TLDR -a openhands` | No |
| **Qwen Code** | `npx skills add ZeroPointNineBar/TLDR -a qwen-code` | No |
| **Atlassian Rovo Dev** | `npx skills add ZeroPointNineBar/TLDR -a rovodev` | No |
| **Tabnine CLI** | `npx skills add ZeroPointNineBar/TLDR -a tabnine-cli` | No |
| **Trae** | `npx skills add ZeroPointNineBar/TLDR -a trae` | No |
| **Warp** | `npx skills add ZeroPointNineBar/TLDR -a warp` | No |
| **Replit Agent** | `npx skills add ZeroPointNineBar/TLDR -a replit` | No |
| **JetBrains Junie** *(soft probe)* | `npx skills add ZeroPointNineBar/TLDR -a junie` | No |
| **Qoder** *(soft probe)* | `npx skills add ZeroPointNineBar/TLDR -a qoder` | No |
| **Google Antigravity** | `node bin/install.js --only antigravity` *(or `npx -y github:ZeroPointNineBar/TLDR -- --only antigravity`)* | Yes (global `~/.gemini/config/AGENTS.md` + skill) |
...
```bash
# Clone the repo
git clone https://github.com/ZeroPointNineBar/TLDR.git
cd TLDR
...
curl -fsSL https://raw.githubusercontent.com/ZeroPointNineBar/TLDR/main/src/rules/tldr-activate.md \
...
- Skills installed via `npx skills add` — the `skills` CLI manages those. Run `npx skills remove ZeroPointNineBar/TLDR` (or use your IDE's skill manager).
...
Still broken? [Open an issue](https://github.com/ZeroPointNineBar/TLDR/issues).
...
Stuck? Open an issue: <https://github.com/ZeroPointNineBar/TLDR/issues>
