...
│
├── install-full.sh / install.ps1 # shims → bin/install.js
│
...
The `install-full.sh` / `install.ps1` shims at the repo root delegate to `bin/install.js` via `node` (local clone) or `npx -y github:jqbit/TLDR` (curl|bash). No legacy fallback path remains — earlier `install.sh.legacy` / `install.ps1.legacy` files were removed.
...
- `bin/install.js` is the only full-agent installer source. `install-full.sh` / `install.ps1` at repo root are shims that delegate to it. Root `install.sh` remains the lightweight prompt copier. Never re-add per-OS install logic to the shims — that's how we got the Windows quoting bug (#249).
